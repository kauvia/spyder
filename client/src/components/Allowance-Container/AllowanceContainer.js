...
class AllowanceContainer extends Component {
    constructor(props){
        ...
        calculateAllowance() {
            ...
            if (currentStat.gender === 'male') {
                bmrString = 66 + currentStat.weight*6.23*2.205 + currentStat.height*12.7/2.54 - currentStat.age*6.8;
                bmr = parseFloat(bmrString);
            } else {
                bmrString = 655 + currentStat.weight*4.35*2.205 + currentStat.height*4.7/2.54 - currentStat.age*4.7;
                bmr = parseFloat(bmrString);
            }
    ...
    render() {
        if (this.props.statHistory) {
                ...
                return (
                <div>
                    {this.state.allowance} - {this.state.food} + {this.state.exercise} = {this.state.left} <br />
